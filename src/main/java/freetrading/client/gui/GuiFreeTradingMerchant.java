...
public class GuiFreeTradingMerchant extends GuiScreen {
	...
	private final static String[] ROMAN_NUMERALS = new String[] { "0", "I", "II", "III", "IV", "V", "VI", "VII",
			"VIII" };
	...
	public GuiFreeTradingMerchant(EntityPlayer playerIn, EntityVillager theMerchantIn) {
		...
		for (int i = 0; i < 3; ++i) {
			for (int j = 0; j < 9; ++j) {
				int index = j + i * 9;
				merchantInventorySlots[index] = new GuiInventorySlotTradeOffer(xMargin + j * 18,
						yMerchantSlotsMargin1 + i * 18, merchantInventory.tradeOffers, index);
			}
		}
		for (int i = 0; i < 3; ++i) {
			for (int j = 0; j < 9; ++j) {
				int index = j + i * 9 + 9;
				playerInventorySlots[index] = new GuiInventorySlotTradeOffer(xMargin + j * 18,
						yPlayerSlotsMargin1 + i * 18, merchantInventory.playerTradeOffers, index);
			}
		}
		for (int k = 0; k < 9; ++k) {
			playerInventorySlots[k] = new GuiInventorySlotTradeOffer(xMargin + k * 18, yPlayerSlotsMargin2,
					merchantInventory.playerTradeOffers, k);
		}
	}
	...
	private GuiInventorySlotTradeOffer getSlotFromPos(int x, int y) {
		...
		if (y > yPlayerSlotsMargin2) {
			int index = x / 18;
			if (index >= PLAYER_SLOT_NUM || index < 0)
				return null;
			...
			if (index >= MERCHANT_SLOT_NUM || index < 0)
				return null;
	...
	public void initGui() {
		...
		merchantLevelString = I18n.format("freetrading.merchant_level",
				merchantLevel >= ROMAN_NUMERALS.length ? merchantLevel : ROMAN_NUMERALS[merchantLevel]);
